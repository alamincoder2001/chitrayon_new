...
class Sales extends CI_Controller {
    ...
    public function addSales(){
        ...
        try{
            ...
            foreach($data->cart as $cartProduct){
                ...
                $colors = $this->db->query("select * from tbl_color where color_name = '$cartProduct->colorName' and status = 'a' ");
                if($colors->num_rows() > 0){
                    $colors = $colors->row();
                    $colorId = $colors->color_SiNo;
                }
                else{
                    $colorInsert = $this->db->query("insert into tbl_color(color_name,status)values('$cartProduct->colorName','a')");
                    $colorId = $this->db->insert_id();
                }

                $pages = $this->db->query("select * from tbl_page where name = '$cartProduct->paperType' and status = 'a'");
                if($pages->num_rows() > 0){
                    $pages = $pages->row();
                    $pageId = $pages->id;
                }
                else{
                    $pageInsert = $this->db->query("insert into tbl_page(name,status)values('$cartProduct->paperType','a')");
                ...
                $saleDetails = array(
                    ...
                    'colorId' => $colorId,
                    'pageId' => $pageId,
                    'longSize' => $cartProduct->longSize,
                    'wideSize' => $cartProduct->wideSize,
                    'bodyDesign' => $cartProduct->bodyDesign,
                    'gsm' => $cartProduct->gsm,
                    'Status' => 'a',
                    'AddBy' => $this->session->userdata("FullName"),
                    'AddTime' => date('Y-m-d H:i:s'),
    ...
    public function getSalesRecord(){
        ...
        if(isset($data->orderType) && $data->orderType != ''){
            $clauses .= " and sm.orderStatus = '$data->orderType'";
        }
        $sales = $this->db->query("
            select 
                sm.*,
    ...
    public function getSales(){
        ...
        if(isset($data->salesId) && $data->salesId != 0 && $data->salesId != ''){
            ...
            $saleDetails = $this->db->query("
                ...
                left join tbl_color c on c.color_SiNo = sd.colorId
                left join tbl_page page on page.id = sd.pageId
                where sd.SaleMaster_IDNo = ?
            ", $data->salesId)->result();
    ...
    function saleReturnDetails()  {
        ...
        $data['title'] = "Orders Return Details";
    ...
    public function updateOrders(){
        ...
        try {
            ...
            $orders = $this->db->query("
                update tbl_salesmaster set orderStatus = '$data->status'
